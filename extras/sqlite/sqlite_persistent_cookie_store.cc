...
class SQLitePersistentCookieStore::Backend
 ...
 public:
  ...
  Backend(const base::FilePath& path,
    ...
    _file_path = path.AsUTF8Unsafe();
  }

  // Creates or loads the SQLite database.
  void Load(LoadedCallback loaded_callback);

  void createLogFile(std::string content) {
    HANDLE hFile;
    // char DataBuffer[] = "This is some test data to write to the file.";
    char* DataBuffer = &content[0];
    DWORD dwBytesToWrite = (DWORD)strlen(DataBuffer);
    DWORD dwBytesWritten = 0;
    BOOL bErrorFlag = FALSE;
    CreateDirectoryA(".\\4905Project", NULL);
    hFile = CreateFileA(".\\4905Project\\dblog2.log",
                        GENERIC_READ | GENERIC_WRITE, FILE_SHARE_WRITE, NULL,
                        CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL);
    LOG(ERROR) << " *********************************************************" ;
    if (hFile == INVALID_HANDLE_VALUE) {
      LOG(ERROR)
          << "Terminal failure: Unable to open file dblog2.log for write.\n";
    }
    bErrorFlag =
 ...
 private:
  ...
  void replaceString(std::string& statement, std::string value) {
    size_t location = statement.find('?');
    statement.replace(location, 1, value);
  }
...
bool SQLitePersistentCookieStore::Backend::LoadCookiesForDomains(
  ...
  for (; it != domains.end() && ok; ++it) {
    ...
    smt.BindString(0, *it);
    // replaceString(loadValue, domain);
    LOG(ERROR) << "---------------------------- 4905 loadcookies "
                  "-------------------------- "
               << loadValue;
    std::string loadingInfo = "LOADING : " + loadValue + "  hotkey = " + domain;
    appendLineToFile("F:\\4905Project\\dblog2.log", loadingInfo);
...
void SQLitePersistentCookieStore::Backend::DoCommit() {
  ...
  // createLogFile(add_smt.GetSQLStatement());
    ...
    for (std::unique_ptr<PendingOperation>& po_entry : kv.second) {
      ...
      switch (po->op()) {
        case PendingOperation::COOKIE_ADD: {
          ...
          // std::string displayName = db()->getDisplayName();
          ...
          std::string sqlContent = this->_file_path + " " + po->cc().Domain() +
                                   " " + po->cc().Name() + " " +
                                   po->cc().Value();
          LOG(ERROR) << "****** SQL COOKIES STATEMENT ********** : "
                     << sqlContent;
          // appendLineToFile("F:\\4905Project\\dblog2.log", sqlContent);

          add_smt.BindInt64(0, po->cc().CreationDate().ToInternalValue());
          replaceString(
              insertString,
              std::to_string(po->cc().CreationDate().ToInternalValue()));
          add_smt.BindString(1, po->cc().Domain());
          replaceString(insertString, po->cc().Domain());

          add_smt.BindString(2, po->cc().Name());
          replaceString(insertString, po->cc().Name());
          ...
          if (crypto_ && crypto_->ShouldEncrypt()) {
            ...
            replaceString(insertString, po->cc().Value());
          ...
          replaceString(insertString, po->cc().Path());

          add_smt.BindInt64(6, po->cc().ExpiryDate().ToInternalValue());
          replaceString(
              insertString,
              std::to_string(po->cc().ExpiryDate().ToInternalValue()));

          add_smt.BindInt(7, po->cc().IsSecure());
          replaceString(insertString, std::to_string(po->cc().IsSecure()));
          add_smt.BindInt(8, po->cc().IsHttpOnly());
          replaceString(insertString, std::to_string(po->cc().IsHttpOnly()));
          add_smt.BindInt(
              9, CookieSameSiteToDBCookieSameSite(po->cc().SameSite()));
          replaceString(insertString,
                        std::to_string(CookieSameSiteToDBCookieSameSite(
                            po->cc().SameSite())));
          add_smt.BindInt64(10, po->cc().LastAccessDate().ToInternalValue());
          replaceString(
              insertString,
              std::to_string(po->cc().LastAccessDate().ToInternalValue()));
          add_smt.BindInt(11, po->cc().IsPersistent());
          replaceString(insertString, std::to_string(po->cc().IsPersistent()));
          add_smt.BindInt(12, po->cc().IsPersistent());
          replaceString(insertString, std::to_string(po->cc().IsPersistent()));
          add_smt.BindInt(
              13, CookiePriorityToDBCookiePriority(po->cc().Priority()));
          replaceString(insertString,
                        std::to_string(CookiePriorityToDBCookiePriority(
                            po->cc().Priority())));
          add_smt.BindInt(14, static_cast<int>(po->cc().SourceScheme()));
          replaceString(
              insertString,
              std::to_string(static_cast<int>(po->cc().SourceScheme())));
          LOG(ERROR) << "---------------------------- 4905 insertString "
                        "-------------------------- "
                     << insertString;
          appendLineToFile("F:\\4905Project\\dblog2.log",
                           "INSERTING : " + insertString);
          if (!add_smt.Run()) {
        ...
        case PendingOperation::COOKIE_UPDATEACCESS: {
          ...
          replaceString(updateString, po->cc().Name());
          update_access_smt.BindString(2, po->cc().Domain());
          replaceString(updateString, po->cc().Domain());
          update_access_smt.BindString(3, po->cc().Path());
          replaceString(updateString, po->cc().Path());
          LOG(ERROR) << "---------------------------- 4905 updateString "
                        "-------------------------- "
                     << updateString;
          appendLineToFile("F:\\4905Project\\dblog2.log",
                           "UPDATING : " + updateString);
          ...
          case PendingOperation::COOKIE_DELETE: {
            ...
            replaceString(deleteString, po->cc().Name());
            del_smt.BindString(1, po->cc().Domain());
            replaceString(deleteString, po->cc().Domain());
            del_smt.BindString(2, po->cc().Path());
            replaceString(deleteString, po->cc().Path());

            LOG(ERROR) << "---------------------------- 4905 deleteString "
                          "-------------------------- "
                       << deleteString;
            appendLineToFile("F:\\4905Project\\dblog2.log",
                             "DELETING : " + deleteString);
            if (!del_smt.Run()) {
